...
function ScenariosPage() {
  const { state, injectDemoDisruption, triggerScenario, isBusy } = useSentinel();
        ...
        >
          <div className="grid gap-2 p-3 sm:grid-cols-2 xl:grid-cols-3">
            {state.shipments.map((s) => (
              <div key={s.id} className="rounded-lg border border-border bg-surface-muted p-3">
                <div className="flex items-center justify-between">
                  <span className="num text-xs font-semibold">{s.id}</span>
                  <Badge tone="neutral">live + demo</Badge>
                </div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={isBusy(s.id) || state.systemStatus.backend !== "healthy"}
                    onClick={() => injectDemoDisruption(s.id)}
                  >
                    Add disruption
                  </Button>
                  <Button
                  ...
                  >
                    ...
                    {isBusy(s.id) ? "Checking…" : "Check live"}
                  </Button>
                </div>
              </div>
            ))}
        ...
        >
          {runs.length === 0 ? (
            <EmptyState
              ...
              description="Run a shipment above to generate real-time recovery options."
            />
          ) : (
