  ...
  return (
    ...
    >
      <div className="space-y-5 p-4 sm:p-5 lg:p-6">
        ...
        <Panel
          title="1 · Sense"
          subtitle="Check news, weather, ships, and ports"
          bodyClassName="p-3"
        >
          <div className="space-y-3">
            {run && run.state !== "ERROR" ? (
              ...
              <p className="text-xs text-muted-foreground">
                {run?.state === "ERROR"
                  ? "The last check failed. Try again to reconnect and check the live data."
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                onClick={() => injectDemoDisruption(shipment.id)}
              >
                Add demo disruption
              </Button>
              <Button
                size="sm"
                disabled={busy || state.systemStatus.backend !== "healthy"}
                onClick={() => triggerScenario(shipment.id)}
              >
                {busy
                  ? "Checking data…"
                  : run?.state === "ERROR"
