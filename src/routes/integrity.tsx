...
function IntegrityPage() {
  ...
  return <AppShell title="Temperature safety" subtitle="Protect temperature-controlled cargo">
    <div className="space-y-5 p-4 sm:p-5 lg:p-6">
      <div className="grid gap-4 md:grid-cols-3"><Summary icon={Snowflake} label="Protected loads" value={String(shipments.length)} /><Summary icon={ShieldCheck} label="Refused unsafe options" value={String(Object.values(state.runs).reduce((n, run) => n + (run.decision?.refusals.length ?? 0), 0))} /><Summary icon={Thermometer} label="Sensor feed" value="Not configured" warning /></div>
      <div className="rounded-xl border border-info/30 bg-info-surface p-4 text-xs leading-relaxed text-muted-foreground"><strong className="text-info">Safety rules are checked before any plan is applied.</strong> Temperature sensor data is not connected yet, so the site will never pretend that it is live. The saved temperature and handling rules are still enforced.</div>
